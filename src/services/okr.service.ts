...
import { ObjectiveOwner, OkrCycle, ObjectiveStatus, KeyResultType, Prisma } from '@prisma/client';
import type {
...
export class OkrService {
  ...
  static async getOkrAlignment(
    ...
    const individualObjectives = objectives.filter(
      (obj) => obj.ownerType === ObjectiveOwner.INDIVIDUAL,
    );
  ...
  static async getOkrSummary(organizationId: string, cycle: OkrCycle, year: number) {
    ...
    const summary = {
      ...
      completedObjectives: objectives.filter((obj) => obj.status === ObjectiveStatus.COMPLETED)
        .length,
      averageProgress:
        keyResults.length > 0
